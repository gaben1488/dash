...
describe('Оценка управлений', () => {
  ...
  it('называет момент чтения и свой периметр вместо бейджа шапки', async () => {
    ...
    // Периметр периода — свой (п.58б); срез по управлению действует (п.127).
    expect(flat()).toContain('фильтры шапки по периоду на эти числа не действуют');
    expect(flat()).toContain('выбранное управление сужает таблицу');
  });
