...
describe('сезонность', () => {
  ...
  it('квартал подписан и на оси, а не сырым ключом книги', () => {
    // Кварталы ядро считало давно, но короткой подписи у них не было: ось
    // падала на «2026-I», и поквартальная картина читалась как строка кода.
    expect(seasonLabel('2026-I')).toBe('I квартал 2026');
    expect(seasonShortLabel('2026-I')).toBe('I кв. 26');
    expect(seasonShortLabel('2026-IV')).toBe('IV кв. 26');
  });

  it('незнакомый ключ периода показывается как есть, а не прячется', () => {
    expect(seasonLabel('что-то')).toBe('что-то');
    expect(seasonShortLabel('что-то')).toBe('что-то');
  });
