 ...
 *
 * Значения обязаны совпадать с ролями `--method-kp` / `--method-ep` в
 * `index.css`: там их берёт словарь смысла `lib/semantic-color.ts`, здесь —
 * график, которому Recharts местами требует настоящую краску. Дома два,
 * значение одно. Пурпур ЕП однажды уже разъехался молча (здесь стоял розовый
 * `#be185d` / `#f472b6`, в `index.css` — фуксия), и один и тот же способ
 * закупки красился по-разному в круге и в бейдже. Страж расхождения —
 * `semantic-color.test.ts`, он читает оба дома и сверяет их байт в байт.
 */
export const METHOD_COLORS: Record<MethodKey, { light: string; dark: string }> = {
  'КП': { light: '#52525b', dark: '#a1a1aa' },
  'ЕП': { light: '#a21caf', dark: '#e879f9' },
};
