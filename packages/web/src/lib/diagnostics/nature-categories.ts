...
const NATURE_BY_CHECK: Readonly<Record<string, NatureCategoryId>> = {
  ...
  // равняться факту (Y); расхождение — дисциплина ввода исполнителя (поправить
  // ячейку K или Y по договору), а не содержательный ход закупки.
  ep_fact_deviation: 'fill',
  // Канон п.98д (18.08.2026): гигиена текста C/G — дисциплина ввода
  // исполнителя (лишние и невидимые символы, латиница в кириллице, имя
  // подведа не по справочнику); карточка несёт готовые исправления ячеек.
  text_hygiene: 'fill',
  // ── Дефект формул листа ──
