...
export const CHECK_REGISTRY: CheckRegistryEntry[] = [
  ...
  },
  {
    ...
    description:
      ...
      + 'извещения и паузы там нет, досрочное заключение договора законно.',
    severity: 'info',
    origin: 'bi_heuristic',
    scope: 'department',
    kbHint:
  ...
  // ================================================================
  {
    // Имя и тон сведены с Реестром 21.08.2026. Одна и та же строка называлась
    // здесь «Факт суммы без даты» (предупреждение, «заполнить дату»), а в
    // Реестре и в корзине-вкладке — законной стадией «Закупка, проводимая в
    // течение года» (канон п.71 от 14.08.2026). Читатель двух вкладок получал
    // противоположные вердикты об одной строке; побеждает решение владельца.
    ...
    description:
      ...
      + 'по мере необходимости. Строка названа стадией, а не ошибкой; в периодные срезы по дате '
      + 'заключения она не попадает и живёт собственной вкладкой.',
    severity: 'info',
    origin: 'bi_heuristic',
    scope: 'department',
    kbHint:
      'Без даты заключения строку нельзя отнести к кварталу по факту — она видна в годовом срезе и '
      + 'на вкладке «В течение года». Дата в такой строке появляется только если договор всё-таки '
      + 'заключён одним документом: тогда её проставляют, и строка уходит в обычный счёт периода.',
    recommendation:
      'Действий по умолчанию не требуется. Если по строке всё же заключён единый договор — '
      + 'проставить его дату в графе Q, и строка встанет в свой квартал.',
    trustComponent: 'data_quality',
  ...
  },
  {
    ...
    description:
      'Способ закупки — единственный поставщик, обоснование в графе M заполнено, но пункт '
      + 'закона из него не читается: либо формулировка своя, либо она расплывчата («в '
      + 'соответствии с действующим законодательством»). Такая строка не попадает ни в одну '
      + 'степень обоснованности и остаётся вне разбора закупок без торгов.',
    severity: 'warning',
    origin: 'compliance_44fz',
    scope: 'department',
    article44fz: 'ст.93',
    kbHint:
      ...
      + 'существу и всё равно не читаться машинно. Поэтому тон — «уточнить формулировку», а '
      + 'не «обоснования нет»: пустую графу называет отдельная проверка.',
    recommendation:
      'Переписать обоснование так, чтобы в нём стоял пункт части 1 статьи 93 44-ФЗ. Если '
      + 'формулировка верна и повторяется по району — сообщить экономическому управлению: '
      + 'словарь оснований пополняется, и признак по таким строкам погаснет.',
    trustComponent: 'rule_compliance',
