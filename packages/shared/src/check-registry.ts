...
export const CHECK_REGISTRY: CheckRegistryEntry[] = [
  ...
  // ================================================================
  {
    ...
    name: 'Текущая деятельность с заполненной графой программы (ТД-ПМ)',
    description: 'Вид деятельности (F) = текущая, но графа программы (D) заполнена. Возможно, это ошибка заполнения, а не отдельная категория.',
    severity: 'warning',
    origin: 'bi_heuristic',
    scope: 'department',
    kbHint: 'Строка одновременно помечена как текущая деятельность и привязана к программе. Такие строки образуют срез «ТД-ПМ». Либо закупка на деле программная — тогда исправить вид деятельности, либо графа программы заполнена ошибочно — тогда очистить её. До разбора строка двоит смысл разрезов ТД/ПМ.',
    recommendation: 'Сверить с исполнителем: программная строка → исправить вид деятельности (F); ошибочная графа программы → очистить D. Составы срезов — вкладка СВОД, «ТД-ПМ».',
    trustComponent: 'data_quality',
    sourceType: 'new',
  },
  {
    // Id остаётся plan_year_missing (уже записан в issues снапшотов);
    // имя — по решению пользователя 07.08: строки без сроков — это
    // закупки без подтверждённого финансирования.
    id: 'plan_year_missing',
    group: 'data_integrity',
    name: 'Закупка без подтверждённого финансирования',
    description: 'Способ и плановые деньги есть, а года плана (P) нет — сроки не проставлены, финансирование не подтверждено. Формулы листа СВОД считают год строго и строку не видят: официальный лимит занижен ровно на её сумму.',
    severity: 'warning',
    origin: 'bi_heuristic',
    scope: 'department',
