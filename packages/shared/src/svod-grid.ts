 ...
 * «ЭА, ЭЗК…» | «ЕП»).
 *
 * Колонки строки-периода (шапка стр.5-6 листа; имена — ПОСЛЕ переименования
 * владельцем 18.08.2026, канон имён — svod-sheet-names.ts):
 *   D=«ПЛАН, единиц», E=«ФАКТ, единиц», F=«Отклонение, единиц»,
 *   G=«Заключено, %» (доля 0..1; было «Выполнено, %»),
 *   H..K = ПЛАН ФБ/КБ/МБ/ИТОГО (тыс. руб.), L..O = ФАКТ ФБ/КБ/МБ/ИТОГО,
 *   P=«Отклонение, тыс. руб», Q=«Законтрактовано, %» (доля; было «Потрачено, %»),
 *   R..U = ЭКОНОМИЯ ФБ/КБ/МБ/ИТОГО. Расчёт ни у одной колонки не менялся.
 */

import { DEPARTMENT_REGISTRY } from './department-registry.js';
import { SVOD_SWITCH_VALUES, type SvodSwitchValue } from './svod-sheet-names.js';
...
//
// СТАТУС «Расч. экономии» на 18.08.2026 (сверка с дампом живой книги): владелец
// УДАЛИЛ её с листа СВОД вместе с «расчётной экономией без УИО» — ни одной
// ячейки в строках долей больше нет, поэтому calcEconomy у листа СВОД теперь
// всегда undefined. Чтение оставлено: те же колонки живут на листе ШДЮ
// (shdyu-ingest), и удаление типа сломало бы разбор ЕГО книги. Показывать
// расч. экономию как показатель СВОДа нельзя — её там нет.
...
export interface SvodSheetExtras {
  ...
  /**
   * Переключатель периметра: «*» = ТД + ПМ (канонический срез), иначе узкий
   * срез, при котором ВСЕ числа листа другие — все COUNTIFS/SUMIFS листа несут
   * условие `'ГРБС'!F:F = B$1`. Продукт обязан знать положение на момент
   * чтения: без этого исторический ряд снимков молча смешивает срезы
   * (аудит 30.07, №13).
   *
   * Живой лист 18.08.2026: переключатель — ячейка B1 (выпадающий список
   * «Текущая деятельность / Программное мероприятие / *»), расшифровка
   * «* = ТД + ПМ» в A2. Прежнее чтение колонки X (индекс 23) промахивалось
   * мимо листа шириной 21 колонка и отдавало undefined всегда.
   */
  perimeterSwitch?: { value: string; row: number; cell: string };
}
...
export function parseSvodExtras(values: unknown[][]): SvodSheetExtras {
  ...
  // Переключатель периметра. Ищем по ЗНАЧЕНИЮ из списка проверки данных, а не
  // по адресу: адрес владелец уже переносил (было X37, стало B1), и привязка к
  // номеру строки ломается молча — с промахом продукт годами получал undefined
  // и не знал, какой срез снял. Значения списка закрыты (три), поэтому поиск
  // по ним точен. Скан узкий — шапка листа, первые пять строк.
  scan: for (let i = 0; i < Math.min(5, values.length); i++) {
    const row = values[i] ?? [];
    for (let c = 0; c < row.length; c++) {
      const v = String(row[c] ?? '').trim();
      if (SVOD_SWITCH_VALUES.includes(v as SvodSwitchValue)) {
        out.perimeterSwitch = { value: v, row: i + 1, cell: `${colLetter(c)}${i + 1}` };
        break scan;
      }
    }
  }
  // Легаси-раскладка (снимки до 18.08.2026): значение стояло в колонке X
  // отдельной строкой под подписью «ПЕРЕКЛЮЧАТЕЛЬ». Старые дампы должны
  // читаться так же, поэтому запасной проход остаётся.
  if (!out.perimeterSwitch) {
    for (let i = 0; i < values.length; i++) {
      const x = String((values[i] ?? [])[23] ?? '').trim();
      if (x !== '' && !x.includes('ПЕРЕКЛЮЧАТЕЛЬ') && !x.startsWith('* =')) {
        out.perimeterSwitch = { value: x, row: i + 1, cell: `X${i + 1}` };
        break;
      }
    }
  }
