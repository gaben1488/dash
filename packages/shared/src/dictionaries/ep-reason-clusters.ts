...
export const EP_REASON_DICT: Record<EpReasonCluster, EpReasonEntry> = {
  ...
  EP_LOWEST_PRICE: {
    ...
    regex: [
      ...
      // Падежное окончание слова «закупка» свободное: живые книги 19.08.2026
      // (E:/aemr-dumps/book-dumps) дают «Закупке с ЕП по наименьшей цене» —
      // УО стр.2586-2588, № 2918-2920. До правки эти три строки падали в
      // UNMAPPED и получали красное «обоснование не распознано», хотя это та
      // же формулировка, что у 907 строк основного кластера. Хвост «.*
      // наименьшей цене» удерживает правило от захвата чужих строк: у записей
      // вида «…дешевле, чем при закупке у ЕП» цены после предлога нет.
      /закупк[аие]\s+(?:с|у)\s+еп.*наи[мм]ень?шей\s+цене/i,
      // «Закупка ЕП по наименьшей цене» — без предлога (живой лист, 2 строки)
      /закупк[аие]\s+еп\s+по\s+наи[мм]ень?шей\s+цене/i,
    ],
    is_legitimate: true,
