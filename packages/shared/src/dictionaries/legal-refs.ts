 ...
 *
 * ── Состояние на 18.08.2026 (не удалять, не считать мусором) ──
 *
 * Проверено по всем пакетам: из файла живёт только тип `LegalRefId` —
 * его берёт `ep-reason-clusters.ts` для поля `legal_refs`, и через это поле
 * правовые привязки хранятся у каждого кластера обоснования ЕП. Значения же
 * (`LEGAL_REFS`, `parseLegalRef`, `getLegalRef`, `isFederalRef`) сегодня не
 * зовёт никто, кроме стража `legal-refs.test.ts`.
 *
 * Это не мёртвый код в смысле «удалить»: DEADCODE_DISPOSITION_2026-06-05 §W-4
 * предписывал обратное — вонзить справочник в разбор и в подсказку «Источник»,
 * а инвентарь долга 18.08.2026 (раздел «б», п. 5) отнёс судьбу трёх мёртвых
 * словарей к решению владельца: вонзить или снять. До решения файл остаётся
 * предметным знанием без потребителя, а страж не даёт ему тихо разойтись с
 * кластерами ЕП, которые на него ссылаются.
 *
 * Оговорка о `in` в parseLegalRef (SECURITY_REVIEW_2026-06-05 §L-8): проверки
 * `id in LEGAL_REFS` безопасны, потому что идентификатор собирается только из
 * цифр (`44_FZ_93_1_${число}`, `AEMR_112_${1|5|8|11}`) и до имён прототипа
 * дотянуться не может. Появится ветка, собирающая ID из произвольного текста, —
 * менять на `Object.hasOwn`.
 */
...
export const LEGAL_REFS: Record<LegalRefId, LegalReference> = {
  ...
  '44_FZ_93_1_9': {
    ...
    law_type: 'federal',
    law_short: '44-ФЗ',
    article: '93', part: 1, item: 9,
    date_adopted: '2013-04-05',
    title_ru: 'Закупка у единственного поставщика — п. 9 (авария, чрезвычайная ситуация, неотложная помощь)',
    issuer: 'Федеральное собрание РФ',
    official_url: 'http://pravo.gov.ru/proxy/ips/?docbody=&nd=102168247',
  },
  ...
  '44_FZ_93_1_25': {
    ...
    law_type: 'federal',
    law_short: '44-ФЗ',
    article: '93', part: 1, item: 25,
    date_adopted: '2013-04-05',
    title_ru: 'Закупка у единственного поставщика — п. 25 (конкурентная процедура признана несостоявшейся)',
    issuer: 'Федеральное собрание РФ',
    official_url: 'http://pravo.gov.ru/proxy/ips/?docbody=&nd=102168247',
  },
