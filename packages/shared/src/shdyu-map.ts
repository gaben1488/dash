...
export interface SHDYUSummaryData {
  ...
  /**
   * Строки «Доля ЭА» / «Доля ЕП», разобранные ОБЩЕЙ колоночной картой.
   *
   * ОСТОРОЖНО (аудит 30.07): доля здесь — не единственное содержимое.
   * Поверх этих строк на листе наложена мини-таблица «Расч. экономия», и
   * денежные тысячи попадают в числовые поля, задокументированные как
   * «доля 0–1» (у района epSharePct.factKB нёс 23 405,28 тыс. руб.).
   * Долей доверять только там, где потребитель знает колонку доли; деньги
   * брать из calcEconomy ниже, а не отсюда.
   */
  compSharePct: SHDYUBlockMetrics;
  /** См. предупреждение у compSharePct. */
  epSharePct: SHDYUBlockMetrics;
  /**
   * «Расч. экономия» района — мини-таблица, наложенная на строки долей
   * (живые формулы листа: L=(J17−N17)·8 % итого, M — ФБ, O — МБ).
   * КБ на месячном листе ОТСУТСТВУЕТ: его ячейку N занимает формула доли,
   * поэтому kb здесь честно null — лист теряет краевой бюджет (9 401,41
   * тыс. руб. против полной разбивки на СВОД ТД-ПМ), и это дефект листа,
   * а не продукта. Поле есть только у района: у ГРБС лист экономию не считает.
   */
  calcEconomy?: { total: number; fb: number; kb: null; mb: number; row: number };
}
