...
function sheet(): unknown[][] {
  const rows: unknown[][] = [];
  // строка 1: переключатель периметра в B1 (живая раскладка 18.08.2026) и
  // шапка «План минус Факт разбивка» справа — K1, подписи источников с L1.
  const r1 = empty();
  cell(r1, 1, '*');
  cell(r1, 10, 'План минус Факт разбивка');
  cell(r1, 11, 'ФБ');
  rows.push(r1);
  // строка 2: расшифровка переключателя в A и «Остаток к заключ.» в K,
  // числа в L/M/N/O
  const r2 = empty();
  cell(r2, 0, '* = ТД + ПМ\n↑↑↑ ПЕРЕКЛЮЧАТЕЛЬ ↑↑↑');
...
/**
 * Легаси-раскладка снимков до 18.08.2026: переключатель стоял в колонке X
 * (индекс 23) отдельной строкой под подписью. Старые дампы обязаны читаться.
 */
function legacySheet(): unknown[][] {
  const rows: unknown[][] = [];
  // X36:X38 — подпись, значение, расшифровка: значение лежало глубоко в листе,
  // а не в шапке, поэтому узкий скан шапки его не видит и работает запасной ход.
...
describe('parseSvodExtras — ярус, который лист считает сам', () => {
  ...
  it('переключатель периметра читается из B1: снимок знает свой срез (№13)', () => {
    // Живой лист 18.08.2026: список «Текущая деятельность / Программное
    // мероприятие / *» стоит в B1, расшифровка «* = ТД + ПМ» — в A2. Прежнее
    // чтение колонки X промахивалось мимо листа шириной 21 колонка.
    expect(x.perimeterSwitch).toEqual({ value: '*', row: 1, cell: 'B1' });
  });

  it('расшифровка в A2 за значение переключателя не принимается', () => {
    expect(x.perimeterSwitch?.value).not.toContain('ТД + ПМ');
  });

  it('узкий срез в B1 читается так же, как канонический', () => {
    const narrow = sheet();
    narrow[0]![1] = 'Программное мероприятие';
    expect(parseSvodExtras(narrow).perimeterSwitch).toEqual({
      value: 'Программное мероприятие', row: 1, cell: 'B1',
    });
  });

  it('легаси-раскладка (X37) читается запасным проходом', () => {
    expect(parseSvodExtras(legacySheet()).perimeterSwitch).toEqual({
      value: '*', row: 37, cell: 'X37',
    });
  });
});
