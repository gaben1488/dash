...
/** После пересборки снимка появились замечания, которых раньше не было. */
export interface IssuesAppearedEvent {
  kind: 'issues-appeared';
  /**
   * Сколько замечаний ПОЯВИЛОСЬ: их стабильных ключей (issue-identity.ts)
   * не было в прошлой сборке. Не разность счётчиков: сравнение чисел
   * объявляло лавину «новых» после любой просадки базы сравнения
   * (упавший источник, фолбэк на старый снимок) — см. issue-news.ts.
   */
  added: number;
  /** Сколько замечаний всего в новом снимке. */
  total: number;
  /** Разбивка ПОЯВИВШИХСЯ замечаний по строгости. */
