...
function scheduleRefresh(log: RouteLog): void {
  ...
  pending = setTimeout(() => {
    ...
    try {
      ...
      // ФОРМУЛЫ ЧИТАЮТСЯ ЗДЕСЬ И ТОЛЬКО ЗДЕСЬ (решение владельца §22 п.7).
      // Уведомление Drive означает, что книгу ТРОГАЛИ рукой, — а перебивают
      // формулу именно рукой: вставили число поверх, протянули соседнюю
      // строку, стёрли ячейку. Плановый цикл опроса за формулы не платит: он
      // идёт каждые несколько минут по книгам, которых никто не касался.
      // Цена одной такой перечитки — второе обращение к Google по четырём
      // диапазонам (K:K, O:P, R:T, Y:AC), числа — в services/google-sheets.ts.
