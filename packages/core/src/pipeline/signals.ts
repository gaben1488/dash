...
export interface RowSignals {
  ...
  /**
   * Исполнение по плану другого года: год факта (S) не равен году плана (P)
   * при обоих заполненных. Решение владельца п.137(4) от 21.08.2026 —
   * информирование, не замечание: годовой срез решает по плановому году
   * (calc-engine.ts), поэтому переходящий с декабря договор исполняется в
   * новом году, а в его годовое число не входит. Замер 21.08.2026: 7 строк УО
   * на 498,22 тыс. руб., и ни на одной из них не было ни единого признака —
   * положение дел не называлось нигде.
   */
  foreignYearExecution: boolean;
  /**
   * Инициативная заявка: примечание ГРБС (AF) целиком равно маркеру «хотелки»
   * — потребность заявлена, но не подтверждена (канон п.76). Решение владельца
   * п.137(3) от 21.08.2026: «отмечаться должны, но не в риск-списки». Признак
   * отмечает строку, а замечание «не обеспечена финансированием» по ней не
   * рождается — до правки бейдж обещал ровно это, а код обещания не исполнял.
   */
  initiativeRequest: boolean;
}
 ...
 *
 * Два значения добавлены 21.08.2026 (спека консолидации сигналов §6, работы
 * без развилок): стадия «в течение года» и класс «не обеспечена
 * финансированием» уже вычислялись и уже имели собственную вкладку, но в оси
 * состояния названы не были — «в течение года» пряталась под подписью
 * «Исполнение», а «не обеспечена финансированием» падала в «Открыт».
 * Взаимоисключаемость всех значений проверена на живых книгах: строк, попавших
 * сразу в два, — ноль.
 */
export type RowState =
  | 'signed'         // Подписан / исполнен
  | 'has-fact'       // Есть фактические данные
  | 'yearlong'       // Закупка, проводимая в течение года (канон п.71)
  | 'overdue'        // Просрочен
  | 'unfunded'       // Не обеспечена финансированием (канон п.23)
  | 'canceled'       // Отменён
...
export function detectSignals(cells: Record<string, unknown>, today?: Date): RowSignals {
  ...
  //
  // ГЕЙТ ЕП ОСТАВЛЕН СОЗНАТЕЛЬНО — разбор 21.08.2026 по решению владельца
  // п.137(6). Спека консолидации назвала цену гейта: он прячет 212 строк на
  // 31 024,11 тыс. руб., то есть класс показан меньше чем наполовину, и
  // спросила «показывать или объяснить, почему нет». Объяснение: у
  // единственного поставщика плановая дата — мягкий ориентир, извещения и
  // паузы между ним и договором нет, и опережение ориентира ничего о строке
  // не сообщает. Класс стал справкой (род info на всех поверхностях, то же
  // решение владельца) — снять гейт значило бы утроить справку, которая по
  // ЕП не значит ничего, и заодно похоронить те 115 строк, где опережение
  // действительно повод сверить даты с договором. Цена гейта названа вслух в
  // паспорте fact_date_before_plan (@aemr/shared check-registry).
  ...
  // ── Исполнение по плану другого года (решение владельца п.137(4), 21.08.2026) ──
  // Годовой срез решает по графе «Год (план)» (P) и графу «Год (факт)» (S) не
  // спрашивает (calc-engine.ts). Договор, заключённый в январе против
  // декабрьского плана прошлого года, исполняется в новом году, а в его
  // годовое число не входит: деньги потрачены, но названы прошлым планом.
  // Род информационный — считать год по плановому году законно; признак
  ...
  const foreignYearExecution =
    ...
    && planYearNum !== factYearNum;

  // ── Инициативная заявка (канон п.76, решение владельца п.137(3)) ──
  // Примечание ГРБС (AF) целиком равно маркеру «хотелки»: потребность
  // заявлена, но не подтверждена. Признак отмечает строку — а замечание «не
  // обеспечена финансированием» по ней не рождается (гейт в orchestrator.ts).
  const initiativeRequest = isInitiativeMarker(cells['AF']);
 ...
 *   error → signed → canceled → overdue → yearlong → has-fact → unfunded →
 *   finance-delay → near-plan → planning → not-due → open → non-data
 *
...
export function classifyRowState(signals: RowSignals): RowState {
  ...
  // Стадия «в течение года» (канон п.71): суммы факта набегают серией
  // платежей, единой даты заключения нет. Ветвь стоит ВЫШЕ 'has-fact'
  // намеренно: до 21.08.2026 эти строки подписывались «Исполнение», и класс,
  // ради которого заведена отдельная вкладка, в оси не назывался вовсе.
  if (signals.factWithoutDate) return 'yearlong';

  // Промежуточные — есть факт
  if (signals.hasFact) return 'has-fact';

  // Не обеспечена финансированием (канон п.23): рукописной плановой даты нет
  // и производный год пуст вслед за ней. Раньше такая строка падала в 'open'
  // вместе с пустой канвой листа — состояние молчало о том, что уже известно.
  if (signals.planYearMissing) return 'unfunded';
