...
describe('linkRowsToProcedures — связка по коду AG', () => {
  ...
  it('код лота с косой — валидная форма (п.121), а не искажение', () => {
    // До 20.08.2026 «ЭАС205/1-26» числился искажением; слова начальницы
    // (п.121): совместные торги дробятся на лоты, код лота = базовый номер
    // + «/» + номер лота. Парсер (@aemr/shared procedure-ref) читает его
    // штатно, поэтому связка выдаёт распознанный код; matched зависит от
    // наличия ровно этого кода лота в карте мониторинга.
    const withoutLot = linkRowsToProcedures([{ rowKey: 'УО:2101', ag: 'ЭАС205/1-26' }], index);
    expect(withoutLot).toEqual([{ rowKey: 'УО:2101', code: 'ЭАС205/1-26', matched: false }]);

    const lotIndex = buildProcedureIndex([...MONITORING_CELLS, 'ЭАС205/1-26 Поставка продуктов (лот 1)']);
    const withLot = linkRowsToProcedures([{ rowKey: 'УО:2101', ag: 'ЭАС205/1-26' }], lotIndex);
    expect(withLot).toEqual([{ rowKey: 'УО:2101', code: 'ЭАС205/1-26', matched: true }]);
  });
