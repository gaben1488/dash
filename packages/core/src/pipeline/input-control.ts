 ...
 *
 * ЧЕСТНО ПРО ПОТРЕБИТЕЛЕЙ (ревизия мёртвого 20.08.2026, зона В): продовый
 * маршрут записи (server/routes/rows.ts) этот модуль СЕГОДНЯ НЕ вызывает —
 * живые читатели: экспорт @aemr/core (публичный API) и тесты. Модуль не
 * удалён сознательно: это готовый серверный слой валидации для Table Editor
 * (UI_SPEC), и он же — единственный продовый потребитель normalizer-rules.
 * Подключение к PATCH-маршруту — отдельной волной; удаление = переписывать
 * заново при подключении.
 */
