...
describe('computeUnifiedGrid — инварианты §7', () => {
  ...
  it('инвариант 1: ВСЕ = ПМ + ТД по planCount (1 кв)', () => {
  ...
  it('инвариант 1: ВСЕ = ПМ + ТД по planFB (1 кв, КП)', () => {
    // КП 1 кв: строка A (ПМ, 100) + строка C (ТД, 30) = 130; ПМ=100, ТД=30
  ...
  it('факт-суммы/кол-во считаются только при наличии факт-даты (как recalculate.ts)', () => {
    // КП 1 кв факт ФБ: строка A (факт-дата есть) =90, строка C (факт=Х) не вносит → 90
    expect(cell('all', 'kp', 'q1')?.factFB).toBe(90);
    // factCount: A есть факт, C нет факта → КП 1 кв factCount=1
    expect(cell('all', 'kp', 'q1')?.factCount).toBe(1);
    // ЕП 1 кв: строка B факт ФБ=50, есть факт
    expect(cell('all', 'ep', 'q1')?.factFB).toBe(50);
    expect(cell('all', 'ep', 'q1')?.factCount).toBe(1);
  });

  it('числа листа с пробелом-разрядом читаются целиком (канон toNumber, блок А п.1)', () => {
    // «1 234,56» в план-ФБ: старый num() обрывался на пробеле → 1,
    // деньги свода занижались на три порядка.
    const g = computeUnifiedGrid({
      x: [row({ 3: 'X', 5: 'Текущая деятельность', 11: 'ЭА', 13: '2026-02-01', 14: 1, 15: 2026, 16: 'Х', 7: '1 234,56' })],
    });
    const c = g.cells[unifiedKey('x', 'all', 'kp', 'q1')];
    expect(c?.planFB).toBeCloseTo(1234.56);
  });
  ...
  it('инвариант 5: экономия только при gate=да', () => {
    // строка A gate=да эко ФБ=10 → ПМ КП 1 кв economyFB=10
  ...
  it('строка без план-даты, но с план-кварталом — учитывается (квартал+год), месяц = 1-й месяц квартала', () => {
    ...
    // Дата пуста, квартал=2 → строка входит в q2/year; месяц-фолбэк = m4 (1-й месяц 2 кв).
...
describe('reconcileUnified — cross-check vs СВОД ТД-ПМ', () => {
  // Одна строка: ВСЕ КП 1 кв/год planFB=100, planCount=1, factFB=90.
  const rows: Record<string, unknown[][]> = {
    uo: [
...
// ────────────────────────────────────────────────────────────
// чанк F: methodOf слеп к алиасам ЕП, которых нет в реальных данных на
// 2026-07-11 (проверено обходом всех 8 ГРБС × 8 недельных выгрузок — только
// 'ЕП'/'ЭА'), но словарь METHOD_ALIAS_MAP документирует их как реальные
// («ЭЕП — некоторые листы УО», «Ед. поставщик»). Старая methodOf ловила
// только startsWith('ЕП') — латентный баг для будущих/иных данных.
// ────────────────────────────────────────────────────────────

describe('computeUnifiedGrid — methodOf распознаёт ЕП-алиасы (чанк F)', () => {
  const cellFor = (method: string) => {
    const rows: Record<string, unknown[][]> = {
      uo: [row({ 3: 'X', 5: 'Текущая деятельность', 11: method, 13: '2026-02-01', 14: 1, 15: 2026, 16: '2026-02-10', 7: 10, 29: 'нет' })],
    };
    const grid = computeUnifiedGrid(rows);
    return grid.cells[unifiedKey('uo', 'td', 'ep', 'q1')]?.planCount ?? 0;
  };

  it('«Ед. поставщик» (не начинается с «ЕП») попадает в ep-корзину', () => {
    expect(cellFor('Ед. поставщик')).toBe(1);
  });

  it('«ЭЕП» (не начинается с «ЕП») попадает в ep-корзину', () => {
    expect(cellFor('ЭЕП')).toBe(1);
  });

  it('«ЕП (ст.93)» (начинается с «ЕП») по-прежнему в ep-корзине — не регрессия', () => {
    expect(cellFor('ЕП (ст.93)')).toBe(1);
  });

  it('регистр не важен: «еп» → ep-корзина', () => {
    expect(cellFor('еп')).toBe(1);
  });
});
