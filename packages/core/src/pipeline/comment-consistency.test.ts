...
describe('правило (а): этапность при заключённом контракте', () => {
  it('УД r14: «в стадии окончания подачи заявок» в AF при Q=27.04.2026 → карточка', () => {
    ...
    // Переквалификация 21.08.2026 (решение владельца 20.08): не упрёк
    // «обновите комментарий», а справка «сверьте, актуален ли текст» —
    // этапность нередко объясняет, почему поставщик в итоге другой.
    expect(card.action).toContain('сверьте, актуален ли текст');
    expect(card.action).not.toContain('удалите устаревшую этапность');
    expect(card.mechanism).toContain('может объяснять смену поставщика');
    expect(card.mechanism).toContain('справка, а не ошибка');
  });
