...
const ECONOMY_EPSILON = 1; // ₽ — экономия |x|<1 считается нулевой (float/округление)
 ...
 * массово в «предлимитной» полосе (480-600К — чуть ниже потолка 600К), на крупную сумму.
 */
export function detectSplitting(rows: AntiCorruptionRow[]): AntiCorruptionFlag[] {
  ...
  const limit = LAW_44FZ.epSingleContractLimit; // 600 000
  const bandLow = limit * 0.8; // 480 000 — «предлимитная» полоса
  ...
  for (const [subj, group] of groups) {
    ...
    const total = group.reduce((s, r) => s + r.planTotal, 0);
    if (total > 10_000_000) score += 15;
    if (total > 20_000_000) score += 10;
    ...
    if (score > 60) {
      flags.push({
        ...
        score,
        subject: subj,
        message: `Возможное дробление: ${group.length} ЕП на однородный предмет «${subj}» на сумму ${(total / 1_000_000).toFixed(1)} млн ₽ (индекс ${score}/100) — требует проверки`,
      });
    }
  }
  return flags;
  ...
  for (const r of rows) {
    ...
    if (r.planTotal > LAW_44FZ.epSingleContractLimit) {
      flags.push({
        ...
        severity: 'high',
        rowIndex: r.rowIndex,
        message: `ЕП ${(r.planTotal / 1000).toFixed(0)} тыс ₽ > предельных 600 тыс (строка ${r.rowIndex}) — требует проверки основания`,
      });
    }
  }
  return flags;
