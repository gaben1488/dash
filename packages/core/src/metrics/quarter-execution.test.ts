 ...
 * Эталон отчёта на 20.03.2026 (1 кв): УЭР 6/15 = 40.00%, УКСиМП 4/30 = 13.33%.
 */
...
describe('quarterExecution — G = E/D (канон СВОД, отчёт 20.03.2026)', () => {
  it('(а) базовый: 15 строк план 1 кв, 6 с фактом → 6/15 = 40.00', () => {
  ...
  it('(б) строка с фактом, но с планом другого квартала — не в знаменателе (и не в числителе: атрибуция по O)', () => {
    const rows = [
      ...planRows(3, 1, 1),
      // Факт есть, но план — 2 кв: в метрику 1 кв не входит вообще (E ⊆ D по столбцу O).
  ...
  it('накопительно: дата факта с плановой НЕ сравнивается — поздний факт (май) считается для 1 кв', () => {
    const rows = [
  ...
  it('(в) пустой план на квартал → pct = null (честное «нет плана», не 0 и не 100)', () => {
    // Все планы — 2 кв; для 1 кв знаменатель пуст.
  ...
  it('(д) пустой план-год в годовой срез НЕ входит — как на листе (волна 0)', () => {
    const rows = [
      ...planRows(2, 1, 1),
      makeRow({ id: 'no-year', planQuarter: 1, planYear: '', factDate: '20.02.2026' }),
    ];
    const r = quarterExecution(rows, { quarter: 1, year: 2026 });
    // До 08.08 такая строка проходила в срез ЛЮБОГО года («нестрогий гейт»),
    // и лимит управления расходился с листом: УЭР 13 921 против 13 331,23 —
    // пять строк на 589,93 тыс. руб. Лист адресует строку парой «квартал ×
    // год», и строке без года лечь на нём некуда. Теперь правило то же, но
    // без немоты источника: строка попадает в корзину остатка и показывается
    // отдельной строкой «без года плана» (реестр расхождений 08.08 §2).
    expect(r.planCount).toBe(2);
    expect(r.doneCount).toBe(1);
  });
