...
export interface KBEntryData {
  ...
  // ── Карточка 2.0 (спека 2026-08-20-kb-uplift, раздел 1) ──

  /**
   * ⑪ Скоуп и момент — одна фраза из трёх частей: периметр (какие книги,
   * управления, способ закупки), период и время чтения книг.
   *
   * Поле держит ПОСТОЯННУЮ часть ответа — ту, что верна для метрики всегда:
   * «считается по всем восьми книгам», «отбор шапки этот маршрут не
   * принимает». Изменчивую часть — выбранные управления, период и момент
   * чтения — подставляет экран из хранилища; выдумывать её в словаре нельзя.
   */
  scopeMoment?: string;

  /**
   * ⑫ Почему может разойтись с книгой или Сводом — одна фраза: как считает
   * источник, как считаем мы, в чём разница. Честное «лист такого разреза не
   * считает вовсе» — полноценный ответ, а не пустота.
   */
  divergence?: string;
}
