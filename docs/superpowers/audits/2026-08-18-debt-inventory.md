...
| §1-15 | `IssueList.tsx` мёртв | НЕПРИМЕНИМ | файл жив и используется: `packages/web/src/pages/Recs.tsx` его импортирует — это не долг | web | — | — |
...
| D-4…D-10 | удалить семь файлов веба | ЗАКРЫТ | из семи удалены шесть; `IssueList.tsx` оказался живым (`Recs.tsx`) — строку реестра вычеркнуть как ошибочную | web | — | — |
...
**Посылка оказалась ложной (8).** `IssueList.tsx` жив и используется страницей
рекомендаций — строка «мёртв» была ошибкой. Схлопывание сверки (чанк B)
